...
# NOTE: Windows Defender removal used to live here, but running it in the base -
# immediately after the cumulative-update reboot - hit a transient CBS servicing
# conflict that Uninstall-WindowsFeature threw on and the try/catch swallowed, so
# it silently no-op'd. It now runs in the build-server dev-tools step (after VS +
# a reboot, when CBS is idle), where it succeeds reliably. See install-dev-tools.ps1.
